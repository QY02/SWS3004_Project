    ...
    <div v-else>
      <div id="event" v-loading="loading">
        <t-card
            ...
            :key="index"
            :title="item['name'].length > 6 ? item['name'].substring(0, 6) + '...' : item['name']" :subtitle="item.content.length > 18 ? item.content.substring(0, 18) + '...' : item.content" :style="{ width: '445px' }" hover-shadow
